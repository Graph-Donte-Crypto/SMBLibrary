...
namespace SMBLibrary.Client
{
    public class NamedPipeHelper
    {
        public static NTStatus BindPipe(INTFileStore namedPipeShare, string pipeName, Guid interfaceGuid, uint interfaceVersion, out object pipeHandle, out int maxTransmitFragmentSize)
        {
            ...
            NTStatus status = namedPipeShare.CreateFile(out pipeHandle, out FileStatus fileStatus, pipeName, (AccessMask)(FileAccessMask.FILE_READ_DATA | FileAccessMask.FILE_WRITE_DATA), 0, ShareAccess.Read | ShareAccess.Write, CreateDisposition.FILE_OPEN, 0, null);
            ...
            ContextElement serviceContext = new()
            {
            ...
            if (RPCPDU.GetPDU(output, 0) is not BindAckPDU bindAckPDU)
            {
