...
namespace SMBLibrary.Win32.Security
{
    ...
    public class LoginAPI
    {
        ...
        {
            bool success = LogonUser(userName, String.Empty, password, (int)logonType, LOGON32_PROVIDER_WINNT40, out nint token);
            if (!success)
            {
        ...
        {
            bool success = LogonUser(userName, String.Empty, String.Empty, (int)LogonType.Network, LOGON32_PROVIDER_WINNT40, out nint token);
            if (success)
            {
                CloseHandle(token);
                return true;
